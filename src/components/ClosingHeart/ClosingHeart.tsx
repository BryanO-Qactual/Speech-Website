...
import contentArray from './ClosingHeartContent'
import "./ClosingHeart.css"

const ClosingHeart = () => {
    const storageName = "ClosingHeart"
    const CopySecName = "CopiedClosingHeart"
    const [initialRender, setInitialRender] = useState(true);
    // SECTION - values of each text area 
    const {closingHeart: [val, setVal]} = useContext(MainContext) || {}
    
    // SECTION - Makes text area resize automatically if they write a lot
    const textAreaRefs = getTextAreaRefs(4)
  ...
  return (
    <div className={"ClosingHeartContainer newpage"}>
        <div>
            <h1 className={"ClosingHeartTitle"}>CLOSING HEART</h1>
